...
cor_matrix_rawData_pytania <- round(cor(rawData_pytania), 2)
...
**W większości słabe korelacje, czy to źle??**
...
*W naszych projektach od 0.7 powinno być* -- **JEST 0.62: CZY TO ŹLE??**
...
* raw_alpha = 0.6222536 (surowy współczynnik alfa Cronbacha)
* std.alpha = 0.6367148 (standaryzowany alfa Cronbach -- wartość bardzo zbliżona do surowej alfy, gdy wszystkie zmienne są na tej samej skali)
* G6(smc) = 0.7403259 (Lambda 6 Guttmana)
* average_r = 0.06805738 (średnia korelacja między zmiennymi; wskazuje, jak silnie zmienne są ze sobą powiązane -- im wyższa, tym bardziej jednolity test)
* S/N = 1.752658
* mean = 4.159066 (średnia wartość odpowiedzi w skali testu)
* sd = 0.5273359 (odchylenie standardowe wyników testu)
* median_r = 0.06611555 (mediana korelacji między pytaniami)
...
## Czy Alfa Cronbach jest odpowiedni do naszych danych?
...
[Reliability if an item is dropped -- jak zmieni się rzetelność testu (alfa Cronbacha), gdy usuniemy którąś ze zmiennych (np. gdy wyrzucimy x3, to Cronbach wzrośnie do 0.87, czyli x3 jest mało ważny).] -- nie wiem do której części kodu to było na zajęciach opisane

```{r Lambdy i beta dla rawData_pytania, warning = FALSE, message = FALSE}
splitHalf(rawData_pytania)
```

Gdyby beta (minimum) była wysoka to oznaczałoby, że jest jeden konstrukt (wszystkie są skorelowane) -- wtedy Cronbach. 

U nas `Minimum split half reliability  (beta) = 0.53` -- najniższa korelacja między połówkami zbiorów. **(to duża czy mała?)**
...
```{r Omega McDonald}
omega_result <- omega(rawData_pytania, fm = "ml", plot = FALSE)
omega_result
```
